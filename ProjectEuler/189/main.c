...
long long getIndex(int level[], int c) {
    long long ret = 0;
    ...
    if (n > 1) {
        ...
        for (size_t j = 2; j <= n - 1; j++) {
            if (0 < j - 2) {
                ...
                memset(now, 0, arry_size * sizeof(int));
            }

            for (size_t i = 0; i < j; i++) {
                now_level[i] = 0;
            }
            for (size_t i = j; i < 14; i++) {
                now_level[i] = -1;
            }
